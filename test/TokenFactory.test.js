...
describe("TokenFactory", function () {
  ...
  describe("burn (LaunchedToken)", function () {
    it("lets a holder permanently destroy their own tokens — totalSupply actually decreases", async function () {
      const { factory, creator } = await deployStack();
      const { token } = await createJustLaunch(factory, creator);
      const supplyBefore = await token.totalSupply();
      const burnAmount = supplyBefore / 10n;

      await expect(token.connect(creator).burn(burnAmount)).to.emit(token, "TokensBurned").withArgs(burnAmount);

      expect(await token.totalSupply()).to.equal(supplyBefore - burnAmount); // true burn, not a transfer to a dead address
      expect(await token.balanceOf(creator.address)).to.equal(supplyBefore - burnAmount);
      expect(await token.balanceOf("0x000000000000000000000000000000000000dEaD")).to.equal(0n); // nothing routed through a dead address
    });
    ...
    it("reverts a burn larger than the caller's own balance", async function () {
      ...
      await expect(token.connect(creator).burn(balance + 1n)).to.be.reverted;
    });
    ...
    it("burnFrom spends the caller's allowance and burns from the approved account", async function () {
      ...
      const { token } = await createJustLaunch(factory, creator);
      const supplyBefore = await token.totalSupply();
      const burnAmount = supplyBefore / 20n;

      await token.connect(creator).approve(otherAccount.address, burnAmount);
      await expect(token.connect(otherAccount).burnFrom(creator.address, burnAmount))
        .to.emit(token, "TokensBurned")
        .withArgs(burnAmount);

      expect(await token.totalSupply()).to.equal(supplyBefore - burnAmount);
      expect(await token.allowance(creator.address, otherAccount.address)).to.equal(0n);
    });

    it("burnFrom reverts without a sufficient allowance", async function () {
      const { factory, creator, otherAccount } = await deployStack();
      const { token } = await createJustLaunch(factory, creator);
      await expect(token.connect(otherAccount).burnFrom(creator.address, 1n)).to.be.reverted;
    });

    it("a burn on a taxed, liquidity-added token is never taxed — the caller's full amount is destroyed, nothing skimmed", async function () {
      const { factory, router, creator, otherAccount: buyer } = await deployStack();
      const { token } = await createWithLiquidity(factory, creator, { liquidityEth: ethers.parseEther("2") });
      expect(await token.taxActive()).to.equal(true); // sanity: the tax is genuinely live for this token
      ...
      expect(buyerBalance).to.be.greaterThan(0n);
      const supplyBeforeBurn = await token.totalSupply();
      const burnAmount = buyerBalance / 2n;

      await token.connect(buyer).burn(burnAmount);

      // Exactly burnAmount destroyed — no tax skimmed off a burn the way one
      // would be off a sell (to == address(0) here, never `pair`).
      expect(await token.balanceOf(buyer.address)).to.equal(buyerBalance - burnAmount);
      expect(await token.totalSupply()).to.equal(supplyBeforeBurn - burnAmount);
    });
  });
