...
describe("PlatformRewardsDistributor", function () {
  ...
  describe("triggerTokenBuyback", function () {
    ...
    // Regression: the direct-credit branch used to read this contract's
    // WHOLE platformToken.balanceOf(address(this)) and split that, but a
    // prior call already split-and-burned an earlier deposit, leaving its
    // still-unpaid airdrop half sitting in that same balance. Re-splitting
    // the whole thing here would burn part of an amount already promised
    // to pendingAirdropTokens and inflate that counter past what this
    // contract can actually pay out later.
    it("only processes the NEW platformToken since the last direct-credit call, not the already-pending leftover", async function () {
      const { distributor, owner, deployer, platformToken } = await deployStack();
      await distributor.connect(owner).setPlatformToken(await platformToken.getAddress());

      const firstAmount = ethers.parseEther("1000");
      await platformToken.connect(deployer).transfer(await distributor.getAddress(), firstAmount);
      await distributor.triggerTokenBuyback(await platformToken.getAddress(), 0);
      const pendingAfterFirst = await distributor.pendingAirdropTokens();
      expect(pendingAfterFirst).to.equal(firstAmount / 2n); // 500 — the unburned half, still sitting on this contract

      const secondAmount = ethers.parseEther("1000");
      await platformToken.connect(deployer).transfer(await distributor.getAddress(), secondAmount);
      // Total balance is now 500 (leftover) + 1000 (new) = 1500 — only the
      // 1000 that's actually new should be split this call.
      await expect(distributor.triggerTokenBuyback(await platformToken.getAddress(), 0))
        .to.emit(distributor, "DirectPlatformTokensProcessed")
        .withArgs(secondAmount, secondAmount / 2n, secondAmount - secondAmount / 2n);

      const pendingAfterSecond = await distributor.pendingAirdropTokens();
      expect(pendingAfterSecond).to.equal(pendingAfterFirst + (secondAmount - secondAmount / 2n));
      // The contract's real balance must always be enough to cover what it
      // now claims is owed — this is exactly the invariant whose breakage
      // used to brick processAirdropBatch with an ERC20InsufficientBalance
      // revert partway through a round.
      expect(await platformToken.balanceOf(await distributor.getAddress())).to.equal(pendingAfterSecond);
    });

    it("reverts (below threshold) rather than double-processing when nothing new has arrived since the last direct credit", async function () {
      const { distributor, owner, deployer, platformToken } = await deployStack();
      await distributor.connect(owner).setPlatformToken(await platformToken.getAddress());
      const amount = ethers.parseEther("1000");
      await platformToken.connect(deployer).transfer(await distributor.getAddress(), amount);
      ...
      // No new platformToken arrived since — calling again must not touch
      // the already-accounted-for leftover half.
      await expect(distributor.triggerTokenBuyback(await platformToken.getAddress(), 0)).to.be.revertedWith(
        "PlatformRewardsDistributor: below threshold"
      );
    });
  });
  ...
  describe("airdrop rounds", function () {
    ...
    // Regression: a direct platformToken credit received WHILE a round is
    // active used to see the round's own not-yet-paid-out balance as
    // "existing balance" and (before the fix) would have re-split it,
    // inflating pendingAirdropTokens past what's actually left to pay once
    // the active round finishes — bricking processAirdropBatch partway
    // through with an ERC20InsufficientBalance revert.
    it("a direct platformToken credit mid-round only processes the new amount, and the active round still completes cleanly", async function () {
      const ctx = await deployStack();
      const { distributor, owner, deployer, platformToken } = ctx;
      await fundAndBuyback(ctx); // pendingAirdropTokens > 0 from a normal ETH buyback
      const pendingBeforeRound = await distributor.pendingAirdropTokens();
      await distributor.startAirdropRound();
      expect(await distributor.roundActive()).to.equal(true);
      expect(await distributor.pendingAirdropTokens()).to.equal(0);
      expect(await distributor.roundAmountRemaining()).to.equal(pendingBeforeRound);

      // Someone (or some other flow) sends platformToken directly to the
      // distributor mid-round.
      const extra = ethers.parseEther("300");
      await platformToken.connect(deployer).transfer(await distributor.getAddress(), extra);
      await expect(distributor.triggerTokenBuyback(await platformToken.getAddress(), 0))
        .to.emit(distributor, "DirectPlatformTokensProcessed")
        .withArgs(extra, extra / 2n, extra - extra / 2n);
      expect(await distributor.pendingAirdropTokens()).to.equal(extra - extra / 2n);

      // The active round is untouched by the mid-round credit and can still
      // fully complete without ever trying to pay out more than this
      // contract actually holds.
      await expect(distributor.processAirdropBatch(1000)).to.emit(distributor, "AirdropRoundCompleted");
      expect(await distributor.roundActive()).to.equal(false);
      expect(await distributor.roundAmountRemaining()).to.equal(0);

      // Whatever this contract now claims is still owed (queued for the
      // next round) must never exceed what it actually holds.
      const finalPending = await distributor.pendingAirdropTokens();
      const finalBalance = await platformToken.balanceOf(await distributor.getAddress());
      expect(finalBalance).to.be.gte(finalPending);
    });
  });
});