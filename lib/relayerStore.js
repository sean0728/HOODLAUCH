...
function upsertPendingDeposit(kind, voucherHash, patch) {
...
function removePendingDeposit(voucherHash) {
  const pending = readPendingDeposits();
...
function setPlatformConfig(config, meta) {
  ensureRootDir();
  fs.writeFileSync(
