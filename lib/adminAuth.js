...
//
// The actual timestamp-freshness/signature-recovery mechanics now live in
// lib/signedMessage.js, generalized so the same pattern can verify against
// any expected address (see POST /token-metadata/:tokenAddress in
// scripts/relayer.js, which verifies against a per-token creator instead of
// this module's single ADMIN_WALLET). Everything below is a thin wrapper
// that preserves this module's exact previous exported names/signatures/
// behavior — POST /active-network and POST /platform-config call these the
// same way they always have.
const { isFreshTimestamp: isFreshTimestampGeneric, verifySignatureFrom } = require("./signedMessage");
...
function isFreshTimestamp(timestamp) {
  return isFreshTimestampGeneric(timestamp, MAX_ADMIN_SIGNATURE_AGE_MS);
}
...
function verifyAdminSignature(message, signature) {
  return verifySignatureFrom(message, signature, ADMIN_WALLET);
}
